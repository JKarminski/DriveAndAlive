...
const MOCK_RECORDS = [
  { track: "Alpejska Przeprawa", time: "2:14.381", rank: 12 },
  { track: "Szutrowy Sprint",    time: "1:58.042", rank: 5  },
  { track: "Nocna Autostrada",   time: "3:01.778", rank: 28 },
];
...
export default function Login(): JSX.Element {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!username.trim() || !password.trim()) { setErr("Wypełnij wszystkie pola."); return; }
      ...
      <div className={styles.page}>
        <div className={`${styles.authCard} glass-card fade-up`}>
          ...
          <div className={styles.recordsSection}>
            <h3 className={styles.recordsTitle}>🏁 Moje rekordy na trasach</h3>
            ...
            <p className={styles.recordsNote}>* Rekordy będą synchronizowane z aplikacją mobilną.</p>
          </div>

          <div className={styles.profileActions}>
            <button className="btn btn-outline" onClick={() => navigate("/settings")}>⚙️ Ustawienia</button>
            <button className="btn btn-ghost" onClick={handleLogout} style={{ color:"var(--danger)" }}>Wyloguj się</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className={`${styles.authCard} glass-card fade-up`}>
        ...
        <div className={styles.authTabs}>
          <button className={`${styles.authTab} ${mode === "login" ? styles.authTabActive : ""}`}
            onClick={() => setMode("login")}>Logowanie</button>
          <button className={`${styles.authTab} ${mode === "register" ? styles.authTabActive : ""}`}
            onClick={() => setMode("register")}>Rejestracja</button>
        </div>

        <h1 className={styles.authTitle}>
          {mode === "login" ? "Witaj z powrotem 👋" : "Dołącz do gry 🏎️"}
        </h1>
        <p className={styles.authSub}>
          {mode === "login"
            ? "Zaloguj się aby zobaczyć swoje wyniki i osiągnięcia."
            : "Stwórz konto i stań się częścią społeczności DriveAndAlive."}
        </p>

        {/* Avatar picker (register only) */}
        {mode === "register" && (
          <div className={styles.avatarPicker}>
            <label className={styles.fieldLabel}>Wybierz avatar</label>
